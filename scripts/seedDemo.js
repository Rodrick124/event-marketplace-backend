...
async function createFixedDemoData() {
  ...
  const organizerPasswordPlain = 'Organizer!1234';
  const organizerPassword = await bcrypt.hash(organizerPasswordPlain, 10);
  ...
  if (!organizerUser) {
    ...
    console.log('Fixed organizer created:', { email: organizerEmail, password: organizerPasswordPlain });
  } else {
  ...
  const attendeePasswordPlain = 'attend!234';
  const attendeePassword = await bcrypt.hash(attendeePasswordPlain, 10);
  ...
  if (!attendeeUser) {
    ...
    console.log('Fixed attendee created:', { email: attendeeEmail, password: attendeePasswordPlain });
  } else {
    ...
    if (attendeeUser && sampleEvent) {
      ...
      if (!sampleReservation) {
        ...
        if (eventUpdate.modifiedCount > 0) {
          sampleReservation = await Reservation.create({
...
async function createUsers({ organizers = 10, attendees = 200 }) {
  const organizerPassword = await bcrypt.hash('Organizer!1234', 10);
  const attendeePassword = await bcrypt.hash('attend!234', 10);
